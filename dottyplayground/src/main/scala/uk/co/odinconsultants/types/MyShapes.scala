...
final case class ::[H <: Index, T <: Indices](head: H, tail: T)
  extends Indices {
...
object Indices {
  ...
  type Contains[Haystack <: Indices, Needle <: Index] <: Boolean =
    Haystack match {
      case head :: tail =>
      head match {
        case Needle => true
  ...
  type RemoveValue[RemoveFrom <: Indices, Value <: Index] <: Indices =
    RemoveFrom match {
      case INil => INil
      case head :: tail =>
      head match {
